...
///
/// Every transition is mirrored into the unified log under subsystem `dev.blacktop.Usage`,
/// category `menu-lifecycle`, so an open/close session can be replayed with
/// `log show --predicate 'subsystem == "dev.blacktop.Usage"'`.
@Observable
final class MenuLifecycleRecorder {
    ...
    @ObservationIgnored
    private let logger = Logger(subsystem: "dev.blacktop.Usage", category: "menu-lifecycle")
